...
void setup_tile_configs(float escala){
    TileConfig padrao = {
        .src_y = 0,
        .src_x = 0,
...
void world_draw_func(World *self, float camera_x){
    ...
    float tile_size_real = 32 * self->escala;
    for (int i = 0; i < self->num_plataforms; i++){
        ...
        if (sprite_atual != NULL){
            ...
            if (p.type == PLAT_TYPE_ONE_WAY){
                ...
                int qtd_blocos = (int)(p.w / tile_size_real);
                if (p.w > 0 && qtd_blocos == 0) qtd_blocos = 1;

                float altura_desenho = cfg.dst_h;
                if (p.type == PLAT_TYPE_BLOCK || p.type == PLAT_TYPE_ICE_FLOOR || p.type == PLAT_TYPE_ICE_BLOCK || p.type == PLAT_TYPE_ICE_FLOOR || p.type == PLAT_TYPE_BAMBOO)
                    altura_desenho = p.h;
                ...
                for (int j = 0; j < qtd_blocos; j++){
                    al_draw_scaled_bitmap(
                        ...
                        tela_x + (j * tile_size_real) + cfg.offset_x, 
                        tela_y,

                        cfg.dst_w, altura_desenho,
                        0
                    );
                }
            }
...
World *world_init(void)
{
    ...
    *w = (World){
    ...
    add_plataform(w, offset_x + (71 * escala), 108 * escala,    // Chao
                    28 * escala, 15 * escala,
                    PLAT_TYPE_DANGER);
    add_plataform(w, offset_x + (69 * escala), 123 * escala,    // Chao
                    32 * escala, 3,
                    PLAT_TYPE_BLOCK_BG);
    ...
    add_plataform(w, offset_x + (107 * escala), 108 * escala,    // Chao
                    28 * escala, 15 * escala,
                    PLAT_TYPE_DANGER);
    add_plataform(w, offset_x + (105 * escala), 123 * escala,    // Chao
                    32 * escala, 3,
                    PLAT_TYPE_BLOCK_BG);
    ...
    add_plataform(w, offset_x + (143 * escala), 108 * escala,    // Chao
                    28 * escala, 15 * escala,
                    PLAT_TYPE_DANGER);
    add_plataform(w, offset_x + (141 * escala), 123 * escala,    // Chao
                    32 * escala, 3,
                    PLAT_TYPE_BLOCK_BG);
    ...
    add_plataform(w, offset_x + (179 * escala), 108 * escala,    // Chao
                    28 * escala, 15 * escala,
                    PLAT_TYPE_DANGER);
    add_plataform(w, offset_x + (177 * escala), 123 * escala,    // Chao
                    32 * escala, 3,
                    PLAT_TYPE_BLOCK_BG);
